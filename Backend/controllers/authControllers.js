          ...
          bcrypt.compare(uniqueString, hashedUniqueString, (err, result) => {
            if (err) {
              ...
              if (result) {
                ...
                res.redirect(`${process.env.CLIENT_URL}/resetpassword/${hashedUniqueString}`);
              } else {
                res.status(404).json({
...
const resetPassword = async (req, res, next) => {
  ...
  const data = await resetverification.findOne({
    uniqueString: hashedUniqueString,
  });
  if (data) {
    await resetverification.deleteOne({ uniqueString: hashedUniqueString });
